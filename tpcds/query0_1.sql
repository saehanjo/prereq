...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 1286
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 24369
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 41905
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 31939
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 33447
            then (select avg(ss_ext_sales_price)
                  from store_sales
...
select * from (select  
 ...
 where
  ...
  ca_county in ('Catawba County','Adams County','Essex County','Morrison County','Blaine County') and
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('ND', 'CA', 'IN')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('KY', 'MS', 'GA')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('NM', 'MI', 'PA')
...
select * from (select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (8,12,4,6,7,9) and
       d_year = 1998 and
       ca_state in ('NC','IL','IN'
                   ,'GA','OK','ND','MN')
...
select * from (select  i_item_id
 ...
 and d_date between cast('1998-03-17' as date) 
 				and (cast('1998-03-17' as date) + 30 days)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select * from (select  i_item_id
 ...
 and d_date between cast('1998-07-16' as date) and (cast('1998-07-16' as date) +  60 days)
...
select * from (select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('2001-07-15' as date) and (cast('2001-07-15' as date) +  60 days)
 and i_manufact_id in (317,181,310,527)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1999-01-05','1999-10-03','1999-11-15')))
...
select * from (select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Highland'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  17947
   and ib_upper_bound   <=  17947 + 50000
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (2002) and
        ((i_category in ('Women','Children','Shoes') and
          i_class in ('swimwear','infants','kids')
         )
      or (i_category in ('Books','Men','Sports') and
          i_class in ('home repair','shirts','pools') 
        ))
...
and d_date between '1999-03-16' and 
        (cast('1999-03-16' as date) + 90 days)
