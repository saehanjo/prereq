create table subquery550808 as SELECT SUBSTR(i_item_desc, 1, 30) AS itemdesc, i_item_sk AS item_sk, d_date AS solddate, COUNT(*) AS cnt FROM store_sales, date_dim, item WHERE ss_sold_date_sk = d_date_sk AND ss_item_sk = i_item_sk AND d_year IN (2000 + 2, 2000 + 1) GROUP BY SUBSTR(i_item_desc, 1, 30), i_item_sk, d_date HAVING COUNT(*) > 4;

create table subquery876215 as SELECT c_customer_sk, SUM(ss_quantity * ss_sales_price) AS csales FROM store_sales, customer, date_dim WHERE ss_customer_sk = c_customer_sk AND ss_sold_date_sk = d_date_sk AND d_year IN (2000 + 2) GROUP BY c_customer_sk;

create table subquery710939 as SELECT MAX(csales) AS tpcds_cmax FROM (select * from subquery876215);

create table subquery518286 as with max_store_sales AS (select * from subquery710939) SELECT * FROM max_store_sales;

create table subquery294959 as with max_store_sales AS (select * from subquery710939) SELECT c_customer_sk, SUM(ss_quantity * ss_sales_price) AS ssales FROM store_sales, customer WHERE ss_customer_sk = c_customer_sk GROUP BY c_customer_sk HAVING SUM(ss_quantity * ss_sales_price) > (95 / 100.0) * (select * from subquery518286);

create table subquery93017 as with frequent_ss_items AS (select * from subquery550808) SELECT item_sk FROM frequent_ss_items;

create table subquery305643 as with frequent_ss_items AS (select * from subquery550808) select * from subquery93017;
