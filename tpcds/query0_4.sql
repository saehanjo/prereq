...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 40607
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 1470
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 35042
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 28887
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 22286
            then (select avg(ss_ext_tax)
                  from store_sales
...
select * from (select  
 ...
 where
  ...
  ca_county in ('Calhoun County','Hancock County','Montgomery County','Lemhi County','Adams County') and
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast('2002-01-10' as date) 
				and (cast('2002-01-10' as date) + 30 days)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_country = 'United States'
  and ca_state in ('ME', 'OH', 'WV')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('IL', 'CO', 'PA')
...
select * from (select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (7,11,8,6,3,12) and
       d_year = 2002 and
       ca_state in ('TX','AL','CO'
                   ,'GA','OR','IA','IN')
...
select * from (select  i_item_id
 ...
 and d_date between cast('1998-01-05' as date) 
 				and (cast('1998-01-05' as date) + 30 days)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
and d_date between '2002-03-11' and 
        (cast('2002-03-11' as date) + 90 days)
...
select * from (select  
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('2000-02-08' as date) - 30 days)
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('brown','azure','dim'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 1999
 and     d_moy                   = 7
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('blanched','indian','salmon','cornflower','lime','plum') and
...
select * from (select   
 ...
 from (
 	...
 	,w_state
 	,w_country
 	,'BARIAN' || ',' || 'AIRBORNE' as ship_carriers
       ,d_year as year
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1998
 	and t_time between 9437 and 9437+28800 
 	and sm_carrier in ('BARIAN','AIRBORNE')
     group by 
        w_warehouse_name
 	,w_warehouse_sq_ft
 	,w_city
...
select * from (select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('2000-01-22' as date) and (cast('2000-01-22' as date) +  60 days)
 and i_manufact_id in (123,904,214,935)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1998-02-24','1998-08-07','1998-11-16')))
...
select * from (select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Providence'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  1787
   and ib_upper_bound   <=  1787 + 50000
...
where ss_item_sk = i_item_sk and
      ...
      ss_store_sk = s_store_sk and
      d_year in (1998) and
        ((i_category in ('Home','Music','Men') and
          i_class in ('tables','rock','accessories')
         )
      or (i_category in ('Books','Children','Sports') and
          i_class in ('self-help','school-uniforms','fitness') 
        ))
