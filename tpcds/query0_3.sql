...
select * from (select  
 ...
 where
  ...
  ca_county in ('Ray County','Evangeline Parish','Rowan County','Blaine County','Hettinger County') and
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('VT', 'MI', 'GA')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('IL', 'ME', 'TX')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('OH', 'TN', 'SD')
...
select * from (select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (9,5,12,11,7,4) and
       d_year = 1999 and
       ca_state in ('IA','AL','TX'
                   ,'GA','MS','CO','OR')
...
select * from (select  i_item_id
 ...
 and d_date between cast('1998-02-13' as date) 
 				and (cast('1998-02-13' as date) + 30 days)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select * from (select  *
 from(select w_warehouse_name
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast ('1998-04-05' as date) - 30 days)
...
from (select avg(ss_list_price) B1_LP
      ...
      where ss_quantity between 0 and 5
        and (ss_list_price between 33 and 33+10 
             or ss_coupon_amt between 3749 and 3749+1000
     ...
     (select avg(ss_list_price) B3_LP
      ...
      where ss_quantity between 11 and 15
        and (ss_list_price between 73 and 73+10
          or ss_coupon_amt between 1592 and 1592+1000
     ...
     (select avg(ss_list_price) B4_LP
      ...
      where ss_quantity between 16 and 20
        and (ss_list_price between 80 and 80+10
          or ss_coupon_amt between 17797 and 17797+1000
...
select * from (select  i_item_id
 ...
 and d_date between cast('2001-07-21' as date) and (cast('2001-07-21' as date) +  60 days)
 and i_manufact_id in (675,720,695,789)
...
select * from (select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast ('2002-03-10' as date)) 
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('light','aquamarine','misty'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 1999
 and     d_moy                   = 7
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('smoke','antique','yellow','red','plum','orchid') and
...
select * from (select   
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 2001
 	and t_time between 30752 and 30752+28800 
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('2001-08-23' as date) 
                  and (cast('2001-08-23' as date) +  30 days) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
select * from (select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('2002-07-17' as date) and (cast('2002-07-17' as date) +  60 days)
 and i_manufact_id in (571,423,499,141)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1999-02-21','1999-08-22','1999-11-02')))
...
select * from (select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Union'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  53841
   and ib_upper_bound   <=  53841 + 50000
...
where ss_item_sk = i_item_sk and
      ...
      ss_store_sk = s_store_sk and
      d_year in (2001) and
        ((i_category in ('Shoes','Men','Children') and
          i_class in ('athletic','sports-apparel','toddlers')
         )
      or (i_category in ('Women','Jewelry','Sports') and
          i_class in ('fragrances','jewelry boxes','guns') 
        ))
