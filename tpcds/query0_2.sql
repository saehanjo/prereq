...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast('1999-08-25' as date) 
                  and (cast('1999-08-25' as date) +  14 days)
...
select * from (select  
 ...
 where
  ...
  ca_county in ('Grand County','Calaveras County','Adams County','Bradley County','Screven County') and
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('TN', 'IL', 'KY')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('GA', 'MS', 'OR')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('ND', 'MO', 'TX')
...
select * from (select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (2,3,11,10,9,5) and
       d_year = 2001 and
       ca_state in ('TX','CO','AR'
                   ,'NM','KY','SD','IN')
...
select * from (select  i_item_id
 ...
 and d_date between cast('1998-05-30' as date) 
 				and (cast('1998-05-30' as date) + 30 days)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select * from (select  i_item_id
 ...
 and d_date between cast('2001-07-09' as date) and (cast('2001-07-09' as date) +  60 days)
 and i_manufact_id in (757,818,748,801)
...
select * from (select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast ('1999-03-30' as date)) 
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('rosy','maroon','papaya'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2002
 and     d_moy                   = 1
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('drab','hot','goldenrod','medium','peru','navajo') and
...
select * from (select   
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1999
 	and t_time between 52067 and 52067+28800 
...
select * from (select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('1999-07-06' as date) and (cast('1999-07-06' as date) +  60 days)
 and i_manufact_id in (317,953,276,639)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1998-01-28','1998-09-11','1998-11-07')))
...
select * from (select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Summit'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  35894
   and ib_upper_bound   <=  35894 + 50000
...
where ss_item_sk = i_item_sk and
      ...
      ss_store_sk = s_store_sk and
      d_year in (1999) and
        ((i_category in ('Women','Electronics','Home') and
          i_class in ('swimwear','dvd/vcr players','blinds/shades')
         )
      or (i_category in ('Music','Sports','Shoes') and
          i_class in ('pop','guns','kids') 
        ))
